...
import { ProcessCfg, ProcessManager } from '../modules/process-manager';
...
export class AddServer implements Disposable {
    ...
    private _currentQCfg: ProcessCfg | null = null;
    ...
    public static update(qcfg: ProcessCfg): void {
    ...
    public async updateServerCfg(qcfg: ProcessCfg, overwrite: boolean): Promise<void> {
        console.log('update server configuration called');
        if (overwrite && this._currentQCfg) {
            ProcessManager.current?.removeCfg(this._currentQCfg.uniqLabel);
        }
        ProcessManager.current?.addCfg(qcfg);
        this.dispose();
    }
